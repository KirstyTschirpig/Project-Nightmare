...
namespace Generation.Nodes
{
    ...
    abstract public class LatentActionNodeBase : SimplexNode
    {
        ...
        private struct RoutineData
        {
            ...
            public GenerationFlow generationFlow;
            public RoutineData(IEnumerator enumerator, GenerationFlow generationFlow) {
        ...
        private GenerationOutput onStart;
        private GenerationOutput onUpdate;
        private GenerationOutput onFinish;
        ...
        protected void Begin(IEnumerator enumerator, GenerationFlow f) {
        ...
        protected void BreakAll() {
            if ( currentCoroutine != null ) {
                ...
                OnBreak();
                if ( !graphStoped ) {
                    onFinish.Call(new GenerationFlow());
                }
            }
        }
        ...
        IEnumerator InternalCoroutine(RoutineData data) {
            ...
            if ( onStart != null ) {
                onStart.Call(data.generationFlow);
            }

            data.generationFlow.BeginBreakBlock(BreakAll);
            while ( data.enumerator.MoveNext() ) {
                while ( graphPaused ) {
                    yield return null;
                }
                if ( onUpdate != null ) {
                    onUpdate.Call(data.generationFlow);
                }
                yield return data.enumerator.Current;
            }
            data.generationFlow.EndBreakBlock();

            parentNode.SetStatus(NodeCanvas.Framework.Status.Resting);
            onFinish.Call(data.generationFlow);
            currentCoroutine = null;
        ...
        abstract protected void OnRegisterDerivedPorts(GenerationNode node);
