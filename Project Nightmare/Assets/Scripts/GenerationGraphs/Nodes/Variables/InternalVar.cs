...
namespace Generation.Nodes
{
    ...
    [ExposeAsDefinition]
    abstract public class RelayValueInputBase : GenerationNode
    {
    ...
    public class RelayValueInput<T> : RelayValueInputBase, IEditorMenuCallbackReceiver
    {
        ...
        void IEditorMenuCallbackReceiver.OnMenu(UnityEditor.GenericMenu menu, Vector2 pos, Port contextPort, object dropInstance) {
            if ( contextPort == null || contextPort.type.IsAssignableFrom(this.relayType) ) {
                menu.AddItem(new GUIContent(string.Format("Variables/Internal/Get '{0}'", identifier)), false, () => { generationGraph.AddGenerationNode<RelayValueOutput<T>>(pos, contextPort, dropInstance).SetSource(this); });
            }
        }
#endif
        ///----------------------------------------------------------------------------------------------
    ...
    abstract public class RelayValueOutputBase : GenerationNode
    {
