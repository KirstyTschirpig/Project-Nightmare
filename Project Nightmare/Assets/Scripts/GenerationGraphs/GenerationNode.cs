...
namespace Generation.Graphs
{
    ...
    abstract public partial class GenerationNode : Node, ISerializationCallbackReceiver
    {
        ...
        public GenerationGraph generationGraph { get { return (GenerationGraph) graph; } }
        ...
        public GenerationOutput[] GetOutputFlowPorts() {
            return outputPorts.Values.OfType<GenerationOutput>().ToArray();
        }
        ...
        public void AssignSelfInstancePort() {
            var instanceInput = inputPorts.Values.OfType<ValueInput>().FirstOrDefault(p => p.IsUnitySceneObject());
            if ( instanceInput != null && !instanceInput.isConnected && instanceInput.isDefaultValue ) {
                var instance = generationGraph.GetAgentComponent(instanceInput.type);
                if ( instance != null ) {
        ...
        public GenerationOutput TryAddFieldDelegateFlowOutput(FieldInfo field, object instance) {
        ...
        public GenerationNode ReplaceWith(System.Type t) {
            var newNode = graph.AddNode(t, this.position) as GenerationNode;
            ...
            set
            {
                ...
                if ( graph != null ) {
                    for ( var i = 0; i < graph.allNodes.Count; i++ ) {
                        var node = graph.allNodes[i] as GenerationNode;
                        if ( node != null ) {
            ...
            if ( clickedPort != null && e.type == EventType.MouseUp ) {
                ...
                if ( rect.Contains(e.mousePosition) ) {
                    ...
                    if ( dragDropMisses == graph.allNodes.Count && clickedPort != null ) {
                        var cachePort = clickedPort;
                        clickedPort = null;
                        var menu = generationGraph.GetFullNodesMenu(e.mousePosition, cachePort, null);
        ...
        public override void OnActiveRelinkEnd(Connection connection) {
            ...
            for ( var i = 0; i < graph.allNodes.Count; i++ ) {
                var otherNode = graph.allNodes[i] as GenerationNode;
        ...
        protected override GenericMenu OnContextMenu(GenericMenu menu) {
            ...
            if ( outputPorts.Values.Any(p => p is GenerationOutput) ) { //breakpoints only work with FlowOutputs
