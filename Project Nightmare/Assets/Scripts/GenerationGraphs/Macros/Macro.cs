...
namespace Generation.Graphs.Macros
{

    [UnityEngine.CreateAssetMenu(menuName = "ParadoxNotion/FlowCanvas/Macro Asset")]
    public class Macro : GenerationScriptBase
    {
        ...
        [NonSerialized]
        public Dictionary<string, GenerationFlowHandler> entryActionMap = new Dictionary<string, GenerationFlowHandler>(StringComparer.Ordinal);
        [NonSerialized]
        public Dictionary<string, GenerationFlowHandler> exitActionMap = new Dictionary<string, GenerationFlowHandler>(StringComparer.Ordinal);
        [NonSerialized]
        ...
        protected override void OnGraphValidate() {
            ...
            if ( inputDefinitions.Count == 0 && outputDefinitions.Count == 0 ) {
                var defIn = new DynamicPortDefinition("In", typeof(GenerationFlow));
                var defOut = new DynamicPortDefinition("Out", typeof(GenerationFlow));
        ...
        public void CallFlowInput(string name) {
            var def = inputDefinitions.FirstOrDefault(d => d.name == name && d.type == typeof(GenerationFlow));
            if ( def == null ) {
            ...
            entryActionMap[def.ID](new GenerationFlow());
        }
