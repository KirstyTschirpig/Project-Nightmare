...
namespace DungeonArchitect.Editors.DungeonFlow
{
    public class GrammarExecRuleNodeRenderer : GrammarNodeRendererBase
    {
        ...
        protected override void DrawNodeContent(GraphNode node, GUIStyle style, Rect bounds)
        {
            var layout = CalcLayoutInfo(node, style);
            ...
            GUI.Label(messageBounds, layout.messageContent, layout.messageStyle);

            var modeBounds = new Rect(bounds.position + new Vector2(modeOffsetX, layout.messageSize.y), layout.modeSize);
            GUI.Label(modeBounds, layout.modeContent, layout.modeStyle);
        }
